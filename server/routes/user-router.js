...
Router.post('/signup', validationfunc, async (req, res) => {
    ...
    try {
        ...
        if (user) {
            ...
            const saveAbleUser = new User({ firstName, lastName, email, password: hashPassword, userName: firstName + new Date().getTime().toString() })
            const result = await saveAbleUser.save()
            result && res.status(200).json("user has been created ");
        }
